...
const STOCK_CODE_CATALOG = [
  ...
  {
    "code": "HID-025",
    "description": "Curva 40 mm esgoto",
    "category": "🔧 Hidráulica (HID)",
    "prefix": "HID"
  },
  {
    "code": "HID-026",
    "description": "Curva 75 mm esgoto",
    "category": "🔧 Hidráulica (HID)",
    "prefix": "HID"
  },
  {
...
export default function App() {
  ...
  async function addStockItem() {
    ...
    const canonical = canonicalizeStockEntry(
      ...
      stockForm.category || entry?.category || "Material"
    );
  ...
  function addStockCategory() {
    ...
    setStockForm((prev) => ({ ...prev, category: normalized, code: getSuggestedStockCode(normalized) || prev.code }));
    setStockNewCategory("");
  }
      ...
      <Modal open={stockModal || stockEditModal} title={stockEditModal ? "Editar material" : "Novo material"} onClose={() => { setStockModal(false); setStockEditModal(false); setEditingStockId(""); setStockCodeFeedback(""); }}>
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_300px] gap-5 items-start">
              <div className="space-y-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1.5 text-sm text-slate-500">Escolha o grupo, deixe o sistema gerar o código e finalize o cadastro com os dados operacionais.</p>
                  </div>
                    ...
                    <Field label="Grupo">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const suggestedCode = getSuggestedStockCode(selectedCategory);
                          setStockForm((prev) => ({
                            ...prev,
                            category: selectedCategory,
                            code: editingStockId ? prev.code : (suggestedCode || prev.code),
                          }));
                        }}
                      >
                ...
                <div className="rounded-[24px] border border-slate-200 bg-white p-5 space-y-4 shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-lg font-semibold text-emerald-800">Código gerado automaticamente</p>
                      <p className="text-sm text-slate-500">O sistema sugere o próximo código livre dentro do grupo selecionado.</p>
                    </div>
                  </div>

                  <Field label="Código do material">
                    <div className="relative">
                      <Input value={stockForm.code} readOnly placeholder="Selecione um grupo para gerar o código" className="pr-12 bg-slate-50 font-semibold tracking-wide" />
                  ...
                  {!stockCodeFeedback && extractStockGroupCode(stockForm.category) && !editingStockId ? (
                    <p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Próximo número disponível para o grupo selecionado: <strong className="text-slate-900">{getSuggestedStockCode(stockForm.category) || "—"}</strong></p>
                  ) : null}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-[24px] border border-sky-200 bg-sky-50 px-4 py-4 text-sm text-sky-900 shadow-sm">
                  <p className="font-semibold">Código gerado pelo sistema.</p>
                  <p className="mt-2 text-sky-800">O padrão é <strong>AAA-000</strong> e o número é calculado automaticamente.</p>
                </div>
          ...
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-sm text-emerald-900">
            <strong>Simples, rápido e padronizado.</strong> O sistema evita duplicidades e mantém os códigos no formato correto.
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={addStockItem} disabled={!stockForm.item || !obraAtual || !extractStockGroupCode(stockForm.category) || !stockForm.code}>{stockEditModal ? "Salvar alterações" : "Salvar material"}</Button>
          </div>
        </div>
      </Modal>
