...
function normalizeStoredAppData(stored) {
  if (!stored || typeof stored !== "object") return null;
  return consolidateLegacyStockData(prepareNextDayData({
    ...initialData,
    ...stored,
...
function createPersistableAppData(appData) {
  const normalizedSource = consolidateLegacyStockData(appData);
...
function Topbar({
  ...
  const mobilePages = ["dashboard", "stock", "diarias", "maintenance", "attendance", "history", "comissionamento"];
...
export default function App() {
  ...
  const stockPickerResults = useMemo(() => {
    ...
    return source.filter((item) => {
      const parsed = parseStockItemLabel(item.item || "");
      return [item.code, item.material, parsed.code, parsed.description, item.category, item.invoice]
        .join(" ")
        .toLowerCase()
        .includes(normalized);
    });
